...
var btnAddbox_binding = function() {
  console.info('sandbox');
  $('#btn-newcategory').bind('click', function(){
    console.info('btn-newcategory click');
    $.colorbox({iframe:true, width:"80%", height:"80%", href:"./addcategory"});
  });
};
...
var btn_search = function() {
  $('#btn-search').bind('click', function(){
    ...
    window.location.replace('/web/feed/FeedCategoryManagement?p=1&ps='+$('#pagesize-block').text()+'&qloc='+loc);
  });
};
...
var btn_sortmode_ok = function() {
    ...
    $('#btn-sortmode-ok').bind('click', function(){
      ...
      $.ajax({
        ...
        window.location.replace('/web/feed/FeedCategoryManagement?qloc='+$('.filter-country-subsection select option:selected').val());
...
var btn_sortmode_cancel = function() {
    ...
    $('#btn-sortmode-cancel').bind('click', function(){
      ...
      $('#btn-sortmode').prop('disabled', false);
      $('#btn-search').prop('disabled', false);
      $('#btn-newcategory').prop('disabled', false);
      $('.result-section').show();
      $('.result-sortingmode-section').empty();
      $('.filter-country-subsection select').attr('disabled', false);
      $('.filter-mtype-subsection select').attr('disabled', false);
...
var buildSortingSection = function() {
  ...
  $('.result-sortingmode-section').find('.sort-block').css('display', 'inline-block');
  $('.result-sortingmode-section').find('.sort-title-block').css('display', 'inline-block');
...
var btn_sortmode = function() {
  $('#btn-sortmode').bind('click', function(){
    ...
    $('#btn-newcategory').prop('disabled', true);
